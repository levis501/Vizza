...
export function registerSettingsHandlers(): void {
    ...
    register('get_current_state', async () =>
        settingsOr(() => getEngineContext().getState(), {})
    );

    register('update_simulation_setting', async (args) =>
        applyIfReady(() =>
            getEngineContext().updateSetting(String(args.setting_name), args.value)
        )
    );

    register('update_simulation_state', async (args) =>
        applyIfReady(() =>
            getEngineContext().updateState(String(args.state_name), args.value)
        )
    );
    ...
    register('update_cursor_strength', async (args) =>
        applyIfReady(() =>
            getEngineContext().updateState('cursor_strength', Number(args.strength))
        )
    );
