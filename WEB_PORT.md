...
- [x] 15 settings, 9-variant `MaskPattern` / 7-variant `MaskTarget`, 13 built-in presets. Presets use `..Settings::default()`, so the house "only what differs" rule applies directly — unlike Gray-Scott's. **Seven** pin `pheromone_decay_rate: 100.0` against a default of 10.0 — this line and the comment in `presets/builtins/slimeMold.ts` both said six, against an "eight that omit it" that listed six names and did not add up; the split is 7 pinned + 2 with their own (Cells 30, Net 400) + 4 inheriting = 13
...
**Three more from the browser check, all in the shared `compute.wgsl`, all about the same control.** Reported from :9994 at 1 M agents on the defaults: every mask pattern visibly changes the picture — so the mask works — but the **Strength** slider "doesn't seem to change anything". Two independent causes for that — 9 and 10, either of which alone would have been enough — plus a third defect found while confirming them, which the same fix closes:

9. **The route the mask is actually visible through ignored Strength and Curve outright.** `sample_combined_map` and `sample_combined_map_fast` returned `trail + raw_mask`. The agents' *sensors* read that, so a mask acts as phantom pheromone and agents pile into its bright regions — at a million agents that is the dominant thing a mask does on screen, and it was structurally deaf to both controls. Only `get_mask_factor`'s three consumers applied them. **Fixed in the shader**, which is a deliberate divergence from the desktop build that changes every masked render — the class of change M3's `advect_strength` blend and M6's dither were both deferred to M14 for. Taken here anyway, because a control that visibly does nothing is a defect rather than a parity nuance, and this milestone's deliverable is a working slime mold. Both samplers change identically; they feed the same two sensors and a divergence would make agents behave differently depending only on which one the caller picked. What changes on screen: at the default strength of 0.5 the mask now pulls agents half as hard as it used to, and at 0 not at all.
10. **The one route Strength *did* control had exactly zero headroom at the defaults.** Strength was folded into the mask value *before* the target branches, which made every branch quadratic in it. For the default Pheromone Deposition target that is `mix(dep, mix(0, 100, mf), mf)`, and the default `pheromone_deposition_rate` is **100.0** — simultaneously that branch's range maximum and the UI's own `max`. The whole expression collapses to `100·(1 − mf + mf²)`: exactly 100 at mf = 0 *and* at mf = 1, minimum 75 at mf = 0.5. So on a hard 0/1 pattern like Checkerboard — the user's case — the deposition rate was 100 at every strength, and on a smooth pattern it could only dip 25% in the mid-tones. **`mask_strength` is now a blend weight rather than a scale**: `mask_value_at` shapes the pattern by `mask_curve` alone, `mask_blend` returns the strength, and every branch is `mix(authored, target, strength)` — monotone in Strength for every target, 0 meaning "exactly what the controls say" and 1 meaning "the mask decides". Fixed in the shader rather than by gating `mask_target` or moving the default, because the cancellation is a property of the branch and not of that one number: `mix(a, b, w)` was being handed the mask as its weight, which is what made the effect vanish at both ends instead of at neither. (Not fixed by lowering `pheromone_deposition_rate`, which is `Settings::default()` and shared with the desktop build; and note the seven presets that pin it to 22–63 had headroom all along, so this was worst on **Default**, which is where a new user starts.)
11. **A Disabled pattern plus a stale Mask Target pinned decay — or diffusion — to zero.** `decay_trail` and `diffuse_trail` blended with a hardcoded weight of **`1.0`**, and `mix(a, b, 1.0)` is just `b`: the branch was an unconditional *override*, not a mask. Once M7's defect 1 made `generate_mask` correctly write 0.0 for a Disabled pattern, `mask_factor` is 0, so the target value is `mix(0, 10000, 0) = 0` and the effective decay rate is **zero** — the trail map saturates to a solid mat and never comes back. Diffusion the same, pinned to no spreading at all. Reachable by ordinary use: the Mask Target selector is hidden while the pattern is Disabled (`SlimeMoldMode.svelte:380`) but the state value persists, so picking a target and then switching the pattern off is enough. Both weights are now the blend from defect 10, and `mask_value_at`/`mask_blend` short-circuit to 0 for a Disabled pattern **without reading `mask_map` at all**, so a stale buffer cannot act even on a build that never runs the pass. `packSlimeMoldSimSize` deliberately still uploads `mask_target` ungated (`settings.ts:1088`): gating it there would fix one symptom in one build while leaving the shader's override in place, and it would make the uniform stop describing the state.

**On the desktop build, and on what was masking what.** All three live in `compute.wgsl`, which is shared, so all three are on the desktop too and the fixes carry to it; **no Rust change is needed** — nothing new is read from the uniform, `SimSizeUniform::new` already uploads `mask_pattern` (`simulation.rs:71`), and no binding or struct moves. But only **11** is *newly observable* in the browser, and it is newly observable precisely because of the defect M7 already fixed: the Rust skips `generate_mask` for Disabled, so `mask_map` there still holds the previous pattern, and the override lands on a mask factor near 1 rather than 0 — decay 10000 instead of 0, a field that vanishes instead of one that saturates. Wrong either way, opposite symptom. **9 and 10 are not masked by anything and never were** — they are simply what the desktop has always done, and the browser check found them because M7's other fixes (all 16 agents stepping, the mask actually clearing) made the mask work well enough to be worth turning the knob on.
...
**Not a defect, but it reads as one — switching mask pattern looks like nothing happened.** Also from the browser check: at 1 M agents on the defaults, changing the pattern appears "very slow, if it is happening at all, even at strength 1". `mask_map` is in fact rewritten the same frame (`refreshMask` runs on a dirty flag). What lingers is the *trail* the old pattern deposited, and the arithmetic is unforgiving:

- `update_agents` deposits `effective_deposition_rate * 0.01` and clamps to 1, so at the default rate of 100 that is **+1.0 — one visit saturates a cell outright**;
- `decay_trail` subtracts `decay_rate * 0.0001`, so at the default decay of 10 that is **0.001 a pass**. A saturated cell therefore needs **1000 passes** to clear, and `decay_frequency` is 1, so that is 1000 frames — about **16.7 s at 60 fps**;
- and `diffuse_trail` at the default diffusion of 100 computes `diffusion_rate = 1.0`, i.e. `new = mean(4 neighbours)`, which *spreads* the saturation rather than removing it.

So the old pattern's ghost outlives its mask by roughly a quarter of a minute, and the seven presets that pin `pheromone_decay_rate: 100.0` clear ten times faster (~1.7 s) than the Default the user was on. Verified against the shader, not inferred. Nothing here is wrong — it is the model's own physics — but the next reader should not go hunting for a stale-buffer bug. "Clear Trails" makes the switch instant.

**Testing note for whoever touches the mask.** Three L3 tests were added for defects 9–11, each checked to fail on the pre-fix shader (and 11's two halves reverted independently). Two things make a smaller test pass happily on a broken mask:

- **Checkerboard's block is a fixed 200 *simulation pixels*** (`gradient.wgsl:71`), not a fraction of the field. On the harness's 64² field the whole map is one block of 1.0 and a checkerboard is indistinguishable from a flat mask — so the Strength test resizes to 512² to get alternation on both axes. Any mask test that wants structure below 200 px needs a different pattern.
- **Pheromone Decay is the only target with no branch in `update_agents`** (its arms are 0, 3, 4, 5, 6), which is what makes it the right target for isolating the sensor route: with it selected and `pheromone_deposition_rate` set to 0, the trail map stays exactly zero and the only path from the mask into an agent's heading is the sampler under test — so strength 0 must leave *every* heading untouched, exactly, rather than statistically.

**Known defects left alone, deliberately:**
- **The mask reaches the agent sensors regardless of `mask_target`.** `sample_combined_map_fast` adds the mask to every sensor reading whatever the target is, so selecting Pheromone Decay still steers agents toward bright mask regions. It now obeys Strength and Curve (defect 9), which was the reachable bug; whether the sensor contribution should be gated on the target at all is a behaviour question about the desktop build's design rather than a defect in it, and changing it would alter every masked render a second time. M14.
...
### M8 — Particle Life
- [x] Port **6 of the 11 shaders** — `compute.wgsl` (280), `init.wgsl` (375, all 11 position and 11 type generators), `vertex`/`fragment`, `fade_vertex`/`fade_fragment` — plus the shared `INFINITE_RENDER_SHADER` through the existing `InfiniteRenderer` **`'texture'` path**, which fit exactly (bindings 0/1/2, entry point, sampler config, and a bit-identical 16-byte `[1,0,0,0]` params buffer). **The wrapper was not extended**, so the four other simulations drawing through it are untouched
- [x] ~~Port `tile_render.wgsl` (250), `post_effect.wgsl`, `force_update.wgsl`, `force_randomize.wgsl`~~ — **four dead shaders, none ported.** `tile_render_pipeline` is created (`:998`) and stored (`:2009`) and `set_pipeline` is never called with it in 4,079 lines; `post_effect` is gated on `needs_post_effects()`, which is `return false` reading no field, over params of `[1,1,1,1]` (an identity transform); and the only two functions that dispatch the force shaders, `update_force_element_gpu` (`:2285`) and `randomize_force_matrix_gpu` (`:2326`), have **zero callers in `src-tauri`**. Every force-matrix write that happens on either build is CPU-side plus a `write_buffer`
- [x] ~~Port `background_render.wgsl`~~ — ported as the render pass's **clear value** instead. Unlike Slime Mold's, it does reach the screen (with traces off the particle pass loads over it), but a full-screen REPLACE draw of one constant colour over a cleared target is bit-identical to a clear of that colour, and folding it in removes a pipeline, a bind group, a uniform and a pass per frame. The Rust computed that colour in *three* separate places
- [x] Port 22 `MatrixGenerator`s + 11 `TypeGenerator`s and `matrix_operations.rs` — **128 lines of code, not 567**: the file is 11 pure functions followed by a 439-line `#[cfg(test)]` module of 16 tests, all ported. `TypeGenerator` turned out to be **GPU** logic (`init.wgsl:327`, eleven `generate_*_type` functions, declaration order = case order 0–10), so it went with the shaders
- [x] ~~Port the eleven `matrix_operations` Tauri commands~~ — **all eleven are unreachable** (`commands/particle_life.rs:9-333`, registered in `main.rs`, called by nothing). The UI reimplemented every one inline in TypeScript, so the TS is the only behaviour any user has had. `InteractionMatrix.svelte` lost **230 lines** of untested index arithmetic to the tested module (754 → 570)
- [x] **Clamp the particle count.** `caps.particleLife` was a flat 500,000 — ten times the desktop UI's own max — against a Rust clamp of 100,000, twice *its* UI. Now `min(50_000, storageBinding/24, maxWorkgroupsPerDim × 64)`, clamped in the control, the mode and the sink, per M7's three-places rule. This is a **compute** ceiling, not memory: 50k particles is 1.2 MB and 2.5 × 10⁹ pair evaluations a frame
- [x] **Test (L1):** 86 tests · **Test (L3):** 15 tests · **Test (L4):** 13 tests
- [ ] **Visible:** particle life with a live interaction matrix — **needs confirming at :9994 in a real browser**; see M3's environment limitation

**The headline defect: all 22 matrix generators were no-ops.**

`update_setting("matrix_generator")` (`simulation.rs:3552`) draws a new matrix with `randomize_force_matrix`, then calls `recreate_bind_groups_with_force_matrix` — which rebuilds a bind group over the *same, untouched* `force_matrix_buffer` — and then `update_sim_params`, which writes `SimParams`, not the matrix. **There is no `write_buffer` on that path at all**, so the new matrix never reaches the GPU. Verified by reading both functions end to end.

On screen: picking any of the 22 generators changes the numbers the matrix editor displays and the particles carry on obeying the previous matrix. It was masked because "Regenerate Matrix" and "Randomize" both go through `randomize_settings` (`:3906`), which *does* upload — so the feature works whenever it is tested the obvious way, and fails only through the selector.

**Other defects fixed rather than reproduced.** Each was reverted, its test confirmed failing, and the fix restored.

1. **`compute.wgsl`'s `SimParams` was one member short of the Rust struct.** It declared `…brownian_motion, aspect_ratio, _pad1, _pad2` where `simulation.rs:79` and `vertex.wgsl` declare `…brownian_motion, particle_size, aspect_ratio, _pad1`. Both are 80 bytes, so **nothing errored** — but this shader's `aspect_ratio` read the CPU's `particle_size` (4.0) and every later member was shifted by one word. Latent only because nothing below reads either field; the comment on that very line ("Screen aspect ratio for cursor distance calculation") invites the next reader to use it. **Fixed in the shared shader with no Rust edit**: same 80 bytes, same binding, same dispatch, no field newly read, so the desktop build is unaffected and is now correct.
2. **Particle 0 was welded to (−1, −1) on every fresh start.** The constructor's `State` literal sets `random_seed: 0` (`:653`), `init.wgsl:278` gives particle *i* the seed `random_seed + i`, and the shader's `hash` has no additive constant — so `hash(0)` is 0 through all three rounds, `random_f32(0)` is 0.0, and `generate_random_position(0)` maps both axes to −1. Seeded randomly now, with the seed injectable for tests.
3. **Six controls were dead, the M7 `position_generator` defect six times over.** `update_state` (`:3656`) has exactly one arm, `"color_scheme"`, and warns-then-returns-`Ok` on everything else — so `cursor_size`, `cursor_strength`, `traces_enabled`, `trace_fade`, `color_scheme_reversed` and `background_color_mode` all moved the UI and changed nothing, with no rollback because the command succeeded. Five of the six had a working `update_setting` arm sitting unused 100 lines up the same file. **Fixed the way M7 fixed it — putting the write where the read is** (`get_state` returns all six; `get_settings` returns none), so `updateParticleLifeState` forwards to the setting sink rather than the UI switching commands.
4. **Raising the species count discarded the interaction matrix.** `set_species_count` (`settings.rs:169`) resizes with zeroes and then, above 2, calls `randomize_force_matrix(&Random)` over *every* entry — ignoring the selected generator and the user's hand-tuned matrix, including the authored `Default`. Never visible upstream, because every reachable caller sends its own preserved matrix immediately afterwards; preserved explicitly here. New cells get a small random value rather than zero, since a zero row and column is a species that does nothing on screen.
5. **`applySettings` could write past the end of the force-matrix binding.** A preset moving `species_count` 4 → 8 uploads 64 floats into a 64-byte buffer. Reallocated before the upload.
6. **The four matrix shift buttons were inverted.** "Shift left" (←) computed `new[i][j] = old[i][j-1]`, which slides the columns *right*, while dispatching `type: 'shiftLeft'`. All four disagreed with their own name and arrow. The pairs are exact inverses, so nothing became unreachable and the defect is invisible unless you watch which way the numbers move.
7. **The colour-scheme pair, both halves.** `updateColorScheme` pushed LUT bytes but never wrote the name into state — M4/M5's defect, where the selection reverts on the next sync — and `updateColorSchemeReversed` used a bare state write that pushes no LUT, which is M7's mirror-image defect. Both fixed as those milestones fixed them.
8. **Two hard validation errors in `render_frame_paused`'s trace branch** (`:2888`): it sets bind groups 0 and 1 and omits group 2, which `vertex.wgsl` statically uses for the camera and viewport; and it binds `blit_pipeline`, whose colour target is the *surface* format, into an `Rgba8Unorm` attachment — while `display_blit_pipeline`, the correctly-formatted twin, is what the live path uses. Pause with Traces on and the frame does not draw. Unrepresentable in the port, which has one draw path setting all three groups and no blit.
9. **"Clear Trails" did nothing and reported success.** It called `clear_trail_texture`, a registry stub resolving `null`. Now a real `clearTrails()` capability — and deliberately **not** routed to `reset_runtime_state` the way Slime Mold's is: Particle Life's `reset_runtime_state` (`:3860`) draws a fresh `random_seed` and re-runs `init.wgsl` over the whole pool, so that wiring would have teleported every particle and duplicated the neighbouring "Regenerate Particles" button. The clear also has to wipe **both** ping-pong halves, because the fade pass reads the half it is not writing and a one-half clear lets the old trail blend straight back at `1 − fade_amount`.
10. **The ± flip-sign button was broken three ways.** It invoked `update_particle_life_setting`, for which **no Tauri command exists**; it passed the argument under `setting`, which the shim does not rename, so the handler's `args.setting_name` was the string `"undefined"`; and it dispatched `{type:'flipSign'}` with no `matrix` field while the parent destructures one. Its ten sibling buttons all take a working path. Command, stub and handler deleted.
11. **`get_species_colors` returned four colours regardless.** Called with no argument, defaulting to 4, so at 5–8 species the matrix editor fell back to white row and column labels.
12. **Three different ranges for one number.** The particle-count drag box advertised `min={1}`, the JS clamped `[1000, 50000]`, the Rust clamped `[1000, 100000]`. Likewise `cursor_strength`'s slider ran 0–20 against a Rust clamp of 10, so its top half was inert; and the physics panel's "Reset to Defaults" wrote `max_distance: 0.01` and `force_beta: 0.3` against a `Settings::default()` of 0.05 and 0.5.
13. **Four position generators were implemented and unreachable.** The picker offered 7 of 11; `RainbowRing`, `ColorBattle`, `ColorWheel` and `RainbowSpiral` are fully implemented at `init.wgsl:71-124` and dispatched at `:283`. All option lists now derive from the engine's arrays, so the two cannot drift again.

**`State::default()` is a decoy — the port takes the constructor's literal at `simulation.rs:650-669`.** `State::default()` (`state.rs:96`) is never instantiated in production and `State::new` is called only from `tests.rs`. They disagree in seven fields, and porting `Default` would have shipped trails **on** by default and particles **400× too small** (`particle_size` 0.01 against the runtime's 4.0), on the wrong colour scheme. This is the single easiest way to ship a wrong-looking Particle Life.

**On `foldDispatch`, which the port deliberately does *not* use here.** M2's convention is to fold a 1D dispatch into 2D above the per-dimension limit, and M7's `num_workgroups` fix was supposed to make that safe. It is not safe *here*: both particle kernels index by a bare `global_id.x` (`compute.wgsl:132`, `init.wgsl:272`) with no stride term at all, so folding would make every `id.y > 0` workgroup overwrite row 0 — **introducing** M7's headline defect rather than applying its lesson. The particle count is capped to `maxWorkgroupsPerDim × 64` instead, so the 1D dispatch always fits. Worth reading before applying the convention mechanically in M9–M12.

**Known defects left alone, deliberately:**
- **`calculate_force` is missing an `abs()`.** `compute.wgsl:80` computes `attraction·(1 − (1+β−2d/r)/(1−β))` where the law is `attraction·(1 − |2d/r − 1 − β|/(1−β))`. Without it the force climbs to **2× attraction** at exactly the interaction cutoff and then drops to zero — maximal attraction at the edge of the radius, and a discontinuity there. Real, and it changes every frame at every setting while rendering no control inoperative: the M3 `advect_strength` / M6 dither class. M14.
- **The O(n²) kernel reads its neighbours out of the same `read_write` binding it writes**, so a neighbour can be read mid-update. Splitting source from destination needs a second binding and therefore a Rust edit. **Consequence: a step is not bit-reproducible**, which is why the determinism assertions run against the freshly-seeded pool rather than after stepping.
- **No spatial partitioning.** `compute.wgsl:144` loops over every particle for every particle, as the shader's own comment admits. At the 50,000 cap that is 2.5 × 10⁹ pair evaluations a frame. Reproduced faithfully — rewriting it would have destroyed the ability to diff against the Rust reference, which is what caught most of the above — but it sets the practical ceiling far below what the control advertises. M9's `grid_clear`/`grid_populate` pattern is the eventual answer.
- **Adaptive resolution is not ported, and this one is visible.** `calculate_resolution_scale` (`:316`) scales the surface by up to 4.27× and `update_resolution` reallocates *five* textures whenever zoom moves 5% — about **1.2 GB mid-gesture** at 1920×1080 and zoom 4, 604 MB of it a 4× MSAA target that no pass ever attaches, and ~170 MB at rest even at zoom 1. Offscreens are fixed at surface size capped to 2048² instead. **Consequence: zooming past 1× magnifies the tile texture, so particles soften where the desktop keeps them crisp.** A *bounded* supersample is the right fix. M14.
- **The two tile-count functions disagreed**, CPU `+8`/min 7–9 (`:2697`) against WGSL `+6`/min 5–7 (`infinite_render.wgsl:71`) — at zoom 1 the CPU issued 81 instances that the shader decoded as a 7-wide grid, five wasted rows above and none to the sides. Fixed by *reusing* `InfiniteRenderer.calculateTileCount`, already pinned against the WGSL by an M3 test, so CPU and GPU agree for the first time. A behaviour change from the desktop, in the right direction.
- **Five dead settings fields**, carried so a desktop preset round-trips: `repulsion_strength` (no setter, no uniform, no UI — worse than M7's `mask_reversed`, which at least had a setter), `min_distance` (has a setter but is absent from `SimParams`; `compute.wgsl` hardcodes 0.001 twice), `edge_fade_strength`, `trail_map_filtering`, and `State::particles` (always empty in production, still serialised on every `get_state`). **The whole edge-fade feature is inert on every build**: `vertex.wgsl:122` hardcodes `grid_fade_factor = 1.0`, making `fragment.wgsl:54-64` unreachable and its multiply a no-op. Do not build a control for it.
- `fragment.wgsl` declares `color_mode` at group 1 binding 1 and never reads it; the four-way Gray18/White/Black/ColorScheme switch it was meant to drive does not exist. `ViewportParams.texture_size` and `InitParams.width`/`height` are uploaded and read by nothing.
- **`save_preset` reaches an unimplemented stub returning `Ok(())`** (`:3810`) on the desktop, and a preset carries `Settings` only, never `State` — so particle count, cursor, traces, colour scheme, generators and particle size are outside the preset system entirely. M13's problem, not M8's.

**A Speed control, added deliberately beyond the desktop build.** `dt` is a `State` field with a working `update_setting` arm (`:3475`) that no widget on either build ever reached, so the time step was frozen at 0.016 for every user. It is now a `0.1×`–`4.0×` multiplier in the Settings fieldset, storing `dt = 0.016 × speed` in the one existing field — no second field, so a preset and a `get_state` round-trip are unaffected. The clamp lives on the `case 'dt':` arm of `updateParticleLifeSetting`, the single funnel every external write passes through (widget, preset, restored session, desktop state document); it reduces rather than rejects, and a non-finite value falls back to 0.016, because a NaN in uniform word 9 would poison every position permanently — `wrap_position`'s `floor` propagates it.

**The ceiling is 4.0× for a reason that is not stability.** The brief for this work assumed explicit Euler would go unstable and that the time-compensated friction (`pow(friction, dt*60)`) would not damp the speed-up. **Both were wrong**, and the measurement is worth keeping:

- **It cannot blow up.** `wrap_position` (`compute.wgsl:97`) is a true modulo, so position is bounded whatever the velocity. 288 runs — 12 extreme settings combinations × multipliers to 100×, then 8 frictions × 21 multipliers — produced zero non-finite values and no escape from the world box.
- **Friction damps the speed-up and then reverses it**, because retention is *exponential* in the step (`f^(60·dt)`) while the impulse is only *linear* (`force · dt`). Steady state gives a per-frame displacement of `F·dt²·a/(1−a)` with `a = f^(60dt)`; substituting `k = 60·dt·ln(1/f)` reduces it to `k²/(eᵏ−1)`, maximal where `2 − 2e^(−k) = k`, i.e. `k ≈ 1.5936`. So the fastest step is at **`speed* = 1.5936 / (0.96 · ln(1/friction))`** — which predicted the measured peak at all eight frictions (0.3 → 1.4×, 0.5 → 2.4×, 0.7 → 4.7×, 0.85 → 10.2×, 0.95 → 32×) and was re-derived independently before being accepted.
- At the shipped default friction of 0.5 the peak is 2.4× and **break-even is 4.54×** — past that, raising Speed makes the simulation *slower*. 4.0× is the largest round multiplier below it, so every value the control offers is genuinely faster than leaving it alone. Advertising the 10× or 100× that stability alone would permit would repeat M8's `cursor_strength` defect, except worse: the top of the range would not be inert, it would run backwards. By 25× the velocity underflows to zero and the simulation freezes.

**Consequence worth knowing before reaching for this control: it is worth only ~1.32× at the default friction. Friction is the real speed lever** — 0.5 → 0.95 alone measured a **12×** increase in per-frame displacement with `dt` untouched (the idealised steady state predicts 18.7×; the gap is the model not sitting at steady state), because raising friction is what moves `speed*` far enough right for a larger step to pay.

**Testing notes for whoever touches this next.**
- **Holding the diagonal makes every matrix operation lossy.** The eleven transforms all preserve the diagonal (self-repulsion is physically meaningful and the panel says so), which the Rust's versions do not — and the Rust's are unreachable, so the diagonal-preserving TS is the only behaviour that has ever shipped. The consequence is that ← then → does **not** return the original matrix. Reconciled as a composed `withPreservedDiagonal(original, transformed)` wrapper rather than a flag on each function, so the Rust semantics stay exactly as the 16 ported tests pin them and the divergence is stated once.
- **Recording a command proves nothing about Particle Life's state writes.** The fake logs the call before the model runs, so an assertion on the log passes against a completely dead arm. Read the value back out of `getState()` instead — `cursor_strength` clamping 20 → 10 is the cheap proof that the arm actually executed.
- **`test_struct_layout_compatibility` (`tests.rs:580`) cannot detect a layout mismatch of any kind.** Its `validate_compute_shader_binding` creates two buffers and returns `Ok(())` — no shader module, no bind group layout, no pipeline. `test_sim_params_size_and_alignment` asserts only that the struct is 80 bytes, checks no field offsets and reads no WGSL, which is exactly why defect 1 survived. The port's L1 equivalent asserts field *offsets* against the shader text.
- **A 40-byte uniform buffer binding is legal**, verified by running Dawn/SwiftShader directly rather than reasoning about the 16-byte rounding rule (44 and 48 are accepted too). `InitParams` is 40 bytes and needs no padding.
...
- [ ] **Particle Life's `calculate_force` is missing an `abs()`** (`compute.wgsl:80`), putting maximum attraction exactly at the interaction cutoff with a discontinuity there. Same class as M3's blend and M6's dither: it changes every frame at every setting and disables no control — see M8
- [ ] **Give Particle Life a *bounded* supersample.** M8 dropped the desktop's adaptive resolution, which reallocated five textures per 5% of zoom (~1.2 GB mid-gesture, 604 MB of it an MSAA target no pass attaches). The cost is that zooming past 1× magnifies the tile texture, so particles soften where the desktop keeps them crisp — the one M8 divergence a user can see
