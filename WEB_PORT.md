...
| d | `read_write` rgba16float, per-pixel RMW | `gray_scott/paint.wgsl:16` | Ping-pong + **copy-through** at the early-outs |
...
| f | `@workgroup_size(1,1,1)` dispatched w×h | `gray_scott/reaction_diffusion.wgsl` | → `(8,8,1)`. At 1920×1080 that is 2,073,600 workgroups → 32,400. Expect 10–50×. Cheapest large win in the project |
...
### M4 — Gray-Scott
- [ ] Remediations **(d)** and **(f)**; first ping-pong; first mouse-paint path
- [ ] Port `reaction_diffusion.wgsl` (337), `noise_seed.wgsl`, `paint.wgsl`, `background_render.wgsl`
- [x] ~~Port `shared/infinite_render.wgsl` (306)~~ — landed in M3 as `engine/render/InfiniteRenderer.ts`. Gray-Scott needs the `fs_main` (storage) entry point as well as `fs_main_texture`, so extend it rather than write a second one
- [ ] 8 settings + 9 built-in presets from `gray_scott/mod.rs:16`
- [ ] **Test (L2):** now passes for `reaction_diffusion` and `paint`
- [ ] **Test (L3):** ping-pong ≡ in-place reference after 10 steps; frame-time budget assertion for the workgroup fix
- [ ] **Visible:** interactive reaction-diffusion
